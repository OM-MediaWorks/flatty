...
export class MultiSourceExecute implements Middleware {

  #gatherer: () => Array<any>

  constructor (gatherer: () => Array<any>) {
    this.#gatherer = gatherer
  }

  async execute(context: QueryContext, next: Function) {
    const sources = [typeof context.store === 'string' ? { type: 'sparql', value: context.store } : context.store, ...this.#gatherer()]
